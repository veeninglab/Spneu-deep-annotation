"""
This script loops over all mapped reads in a SAM file. In this version of the script (process_sam_nomulti.py), multimappers are excluded from analysis. Data produced here is exclusively used for sRNA prediction.
...
Command: python bin/process_sam_nomulti.py [output_folder] iter=XXX
This script is run once before (iter=1) and once after (iter=2) sRNA detection by 'small_frags.py'
...
#			break # for testing script		
		...
		if mode == 'PE':
			...
			if new_pair == old_pair: # Indicates that both start and end have been assigned for this read pair
				...
				if new_mapq <= 1 and old_mapq <= 1: # Indicates that both reads have either poor mapping quality or mapping multiplicity
					if rRNAs[strand][0,start] == 1:
						stillcount += 1
					continue
	...
	if iterno == 1:
		norm_read_no = abs(np.dot(rRNAs['+'][0],all_output[sample]['raw']['plus_start_count'][0])) + abs(np.dot(rRNAs['-'][0],all_output[sample]['raw']['minus_start_count'][0])) + stillcount
